...
void TruckRender() {
	...
	if (truck.mGetFlg == false) {
		// 時間の表示
		DrawFormatString(0, 170, GetColor(255, 255, 255), "納品者が通過するまで:%d秒", (int)((TruckStart + DELI_GET_TIME) - TruckNow));
	}
	// 到着しているなら
	else {
		// 時間の表示
		DrawFormatString(0, 170, GetColor(255, 255, 255), "納品者が来るまで:%d秒", (int)((TruckStart + DELI_PASS_TIME) - TruckNow));
	}
